		...
		layui.each(n, function(a, n) {
			...
			var l = n.children && n.children.length > 0,
				...
				s = o(["<li " + (n.spread ? 'data-spread="' + n.spread + '"' : "") + ">", function() {
					...
					return r.check ? '<i class="layui-icon layui-tree-check">' + ("") + "</i>" : ""
				}(), function() {
					return '<a href="' + (n.href || "javascript:;") + '" ' + (r.target && n.href ? 'target="' + r.target + '"' : "") + ">" + ('<i class="layui-icon layui-tree-' + (l ? "branch" : "leaf") + '">' + ("") + "</i>") + ("<cite>" + (n.name || "未命名") + "</cite></a>")
				}(), "</li>"].join(""));
